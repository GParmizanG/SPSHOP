...
public class ProductServiceImpl implements ProductService{
	...
	public Product updateProductById(Product product, MultipartFile file) {
		...
		if(!ObjectUtils.isEmpty(updatedProduct)) {
			if(!file.isEmpty()) {
				try {
					dbProductById.setImageBytes(file.getBytes());
					productRepository.save(dbProductById);
	...
	public List<Product> searchProduct(String ch) {
		return productRepository.findByProductTitleContainingIgnoreCaseAndIsActiveTrue(ch);
	}
}
