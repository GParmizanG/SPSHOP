...
//	BCryptPasswordEncoder bCryptPasswordEncoder;
	...
	public String homeIndex(Model model) {
		...
		List<Product> latestActiveProducts = productService.findAllActiveProducts("").stream()
				.sorted((p1,p2)->p2.getId().compareTo(p1.getId()))
				.limit(8).toList();
		
		model.addAttribute("latestEightActiveProducts", latestActiveProducts);
	...
	public String product(Model model, 
		...
		if (!search.isEmpty()) {
			allActiveProducts = productService.searchProduct(search);
		} else {
			allActiveProducts = productService.findAllActiveProducts(category);
		}
	...
	{
		// Strict validation — catch nulls, empty strings, and whitespace-only
		boolean nameMissing = user.getName() == null || user.getName().isBlank();
		boolean emailMissing = user.getEmail() == null || user.getEmail().isBlank();
		boolean mobileMissing = user.getMobile() == null || user.getMobile().isBlank();
		boolean passwordMissing = user.getPassword() == null || user.getPassword().isBlank();

		if (nameMissing || emailMissing || mobileMissing || passwordMissing) {
			session.setAttribute("errorMsg", "Name, Email, Mobile and Password are required!");
		...
		if (!isValidPassword(user.getPassword())) {
			session.setAttribute("errorMsg", "Weak Password! Min 8 chars and 1 special symbol required.");
			model.addAttribute("user", user);
			return "register";
		}
		...
		if (userService.existsByEmail(user.getEmail().trim())) {
			session.setAttribute("errorMsg", "This email is already registered! Try logging in.");
			model.addAttribute("user", user);
			return "register";
		}
		...
		if(!ObjectUtils.isEmpty(saveUser)) 
		{
			...
			session.setAttribute("errorMsg", "Server Error! Please try again later.");
			model.addAttribute("user", user);
			return "register";
		}
	}
	...
	public String resetPassword(@RequestParam String token, HttpSession session, Model model) 
	{
		...
		if(ObjectUtils.isEmpty(userByToken)) {
			session.setAttribute("errorMsg", "Your reset link is invalid or has expired. Please request a new one.");
...
//	}
	...
	public String resetPasswordOperation(@RequestParam String token, @RequestParam String password, HttpSession session, Model model) 
	{
		...
		if(ObjectUtils.isEmpty(userByToken)) {
			...
			if (!isValidPassword(password)) {
				model.addAttribute("msg", "Password must be at least 8 characters long and contain at least one special character!");
				model.addAttribute("token", token);
				return "reset-password";
			}
			...
			session.setAttribute("successMsg", "Your security has been updated. The password was successfully changed.");
	...
	private boolean isValidPassword(String password) {
		...
		// Regex for: at least 8 chars, at least one symbol
